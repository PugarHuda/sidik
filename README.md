...
demoable even if the live route is rate-limited or switched off), not a silent
failure; only the `?replay=1` query override is gated to non-production.
Arbitrary addresses are probed live by `/api/live` on the same deployment —
`ENGINE_URL` remains for pointing the site at a separately hosted engine.
